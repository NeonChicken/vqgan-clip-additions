...
default_image_size = 512  # >8GB VRAM (def) 512
if not torch.cuda.is_available():
    default_image_size = 256  # no GPU found
    print("No GPU found!")
elif get_device_properties(0).total_memory <= 2 ** 33:  # 2 ** 33 = 8,589,934,592 bytes = 8 GB
    default_image_size = 120  # <8GB VRAM
    print("Detected less than 8GB ram!")
...
# msg = ""
# with open('lastprompts.txt', 'r+') as f:
#     count = 1
#     for line in f.readlines():
#         msg += f"{line}"
#         count += 1
#     msg += f"{count}. {args.prompts} [{seed}]\n"
# with open('lastprompts.txt', 'w+') as f:
#     f.write(msg)
# print(f"Saved prompt in lastprompts.txt")
